...
from ...precomputed.mesh import (
    UnshardedLegacyPrecomputedMeshSource,
    PrecomputedMeshMetadata,
)
...
class GrapheneUnshardedMeshSource(UnshardedLegacyPrecomputedMeshSource):
    ...
    def exists(self, labels, progress=None):
        ...
        filenames = [self.compute_filename(label) for label in labels]
    ...
    def get_fragment_labels(self, segid, lod=0, level=2, bbox=None, bypass=False):
        ...
        manifest = self.fetch_manifest(
            segid, lod, level, bbox, return_segids=True, verify=False
        )
    ...
    def fetch_manifest(
        self, segid, lod=0, level=2, bbox=None, return_segids=False, verify=True
    ):
        """
        ...
        if level == 2:
            verify = False
        manifest = self.fetch_manifest_remote(
            segid, lod, level, bbox, return_segids, verify
        )
    ...
    def download_segid(self, seg_id, bounding_box, bypass, use_byte_offsets=True):
        ...
        fragiter = tqdm(
            fragments, disable=(not self.config.progress), desc="Decoding Mesh Buffer"
        )
        is_draco = False
    ...
    def get(
        ...
        meshes = []
        for seg_id in tqdm(
            segids, disable=(not self.config.progress), desc="Downloading Meshes"
        ):
