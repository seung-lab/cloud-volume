...
@pytest.mark.parametrize('parallel', (1, 2, True))
@pytest.mark.parametrize('protocol', ('gs', 's3'))
def test_parallel_read(parallel, protocol):
  cloudpath = protocol + '://seunglab-test/test_v0/image'
  ...
  img = vol2[:512,:512,:50]
  assert np.all(data1 == img)

@pytest.mark.parametrize('parallel', (2,))
@pytest.mark.parametrize('protocol', ('gs', 's3'))
def test_parallel_read_shm(parallel, protocol):
  cloudpath = protocol + '://seunglab-test/test_v0/image'
